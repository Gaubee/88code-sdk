...
export function SubscriptionsCard({ account }: Props) {
  ...
  const activeSubscriptions =
    subscriptions?.filter((sub) => sub.subscriptionStatus === '活跃中') ?? []
    ...
    <Card className="mb-6">
      ...
      <CardContent>
        {!subscriptions || subscriptions.length === 0 ? (
          <p className="text-muted-foreground py-4 text-center">暂无订阅</p>
        ) : (
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {activeSubscriptions.map((sub) => (
                  <SubscriptionPlanCard
                    ...
                    variant="detail"
                    resetting={
                      resetMutation.isPending && resettingId === sub.id
                    }
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-muted-foreground text-xs">
                          ${creditLimit} · {sub.startDate?.split(' ')[0]} ~{' '}
                          {sub.endDate?.split(' ')[0]}
                        </div>
                      </div>
                    )
