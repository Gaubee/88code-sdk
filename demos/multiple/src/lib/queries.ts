...
export function useLoginInfo(account: Account | null) {
  ...
  const queryKey = React.useMemo(() => queryKeys.loginInfo(account?.id ?? ""), [account?.id]);
...
export function useDashboard(account: Account | null) {
  const { code88 } = useService();
  const enabled = !!account;
  const queryKey = React.useMemo(() => queryKeys.dashboard(account?.id ?? ""), [account?.id]);
  const queryFn = React.useCallback(async () => {
    if (!account) throw new Error("No account");
    const queries = code88.getQueries(account);
...
export function useApiKeys(account: Account | null) {
  const { code88 } = useService();
  const enabled = !!account;
  const queryKey = React.useMemo(() => queryKeys.apiKeys(account?.id ?? ""), [account?.id]);
  const queryFn = React.useCallback(async () => {
    if (!account) throw new Error("No account");
    const queries = code88.getQueries(account);
...
export function useModelUsage(account: Account | null) {
  const { code88 } = useService();
  const enabled = !!account;
  const queryKey = React.useMemo(() => queryKeys.modelUsage(account?.id ?? ""), [account?.id]);
  const queryFn = React.useCallback(async () => {
    if (!account) throw new Error("No account");
    const queries = code88.getQueries(account);
    const now = new Date();
  ...
  return useMutation({
    ...
    onSuccess: () => {
      if (account) {
        ...
        queryClient.invalidateQueries({ queryKey: queryKeys.codexFreeQuota(account.id) });
      }
    },
  });
}
