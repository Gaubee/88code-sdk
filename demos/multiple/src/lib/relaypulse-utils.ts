...
export function formatRelayPulseTimestampSeconds(
...
export function computeRelayPulseAvailabilityPercent(
  entry: RelayPulseStatusEntry,
): number {
  if (entry.timeline.length === 0) return 0
  const sum = entry.timeline.reduce(
    (acc, point) => acc + (point.availability ?? 0),
    0,
  )
