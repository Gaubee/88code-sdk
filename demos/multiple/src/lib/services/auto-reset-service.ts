  ...
  getScheduler(account: Account): SmartResetScheduler {
    ...
    const scheduler = createSmartResetScheduler({
      fetchSubscriptions: async () => {
        console.log(`[AutoReset] 正在获取账户 ${account.name} 的最新订阅数据...`);
      ...
      logger: {
        info: (msg, ...args) => console.log(`[AutoReset:${account.name}]`, msg, ...args),
        warn: (msg, ...args) => console.warn(`[AutoReset:${account.name}]`, msg, ...args),
        error: (msg, ...args) => console.error(`[AutoReset:${account.name}]`, msg, ...args),
      },
    });
