...
export class SmartResetScheduler {
  ...
  async checkAndExecute(): Promise<SchedulerExecutionResult> {
    ...
    this.log("info", `在重置窗口内: ${currentWindow.hour}:${String(currentWindow.minute).padStart(2, "0")}`);
    ...
    for (const subscription of subscriptions) {
      ...
      // 策略决策
      const strategyDecision = shouldResetSubscription(subscription, currentWindow.hour);
      ...
      // 资格校验
      const eligibility = canReset(subscription, currentWindow.requiredResetTimes);
      ...
      results.push({
        ...
        reason: resetResult.success ? strategyDecision.reason : (resetResult.message ?? "重置失败"),
        skipped: false,
      });
    }
